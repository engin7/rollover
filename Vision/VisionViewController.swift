...
class VisionViewController: VisionUIViewController {
	...
	func recognizeTextHandler(request: VNRequest, error: Error?) {
		var numbers = [String]()
        var numbersInt = [String]()
		var redBoxes = [CGRect]() // Shows all recognized text lines
		var yellowBoxes = [CGRect]() // Shows words that might be serials
		...
		for visionResult in results {
			...
			if let result = candidate.string.extractPhoneNumber() {
				let (range, number, integers) = result
		...
		show(boxGroups: [(color: UIColor.red.cgColor, boxes: redBoxes), (color: UIColor.yellow.cgColor, boxes: yellowBoxes)])
		
		// Check if we have any temporally stable numbers.
		if let sureNumber = numberTracker.getStableString() {
			showString(string: numbersInt)
